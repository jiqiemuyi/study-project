...
module.exports = {
  types: [
    ...
    { value: 'style', name: 'style:    不影响代码含义的更改（空格、格式、缺少分号等）' },
    { value: 'refactor', name: 'refactor: 代码重构（既不是修复bug也不是添加功能）' },
    { value: 'perf', name: 'perf:     性能优化' },
    { value: 'test', name: 'test:     添加测试或修正现有测试' },
    { value: 'build', name: 'build:    构建系统或外部依赖的更改（例如：gulp、broccoli、npm）' },
    { value: 'ci', name: 'ci:       持续集成配置文件和脚本的更改（例如：Travis、Circle、BrowserStack、SauceLabs）' },
